...
/**
 * The signal message processor
 *
 * @package message_signal
 * @copyright  2022 onwards Pascal Hürten (pascal.huerten@th-luebeck.de)
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class message_output_signal extends message_output {

    /**
     * Constructor to add needed properties to the Slack app.
     */
    public function __construct() {
        $this->manager = new message_signal\manager();
    }

    /**
     * Processes the message and sends a notification via signal
     *
    ...
    public function send_message($eventdata) {
        ...
        if (!empty($CFG->noemailever)) {
            ...
            debugging('$CFG->noemailever is active, no signal message sent.', DEBUG_MINIMAL);
            return true;
        }
    ...
    public function config_form($preferences) {
        ...
        // If Signal has not been configured, do nothing.
    ...
    public function process_form($form, &$preferences) {
        return $this->manager->set_user_account($form->account);
    }
    ...
    /**
     * Tests whether the Signal settings have been configured on user level
    ...
    public function is_user_configured($user = null) {
        ...
        return ($this->manager->is_user_account_set($user->id));
    }

    /**
     * Tests whether the Signal settings have been configured
     * @return boolean true if Signal is configured
     */
    public function is_system_configured() {
        return (!empty(get_config('message_signal', 'botname')) && !empty(get_config('message_signal', 'botaccount')));
    }
}
